            ...
            <div className='mx-auto w-full max-w-[440px] px-5 py-[clamp(36px,8vh,80px)]'>
                <div className='grid gap-4 rounded-porch border border-line bg-surface p-[clamp(22px,4vw,34px)]'>
                    ...
                    <div className='flex items-center justify-between rounded-porch bg-surface2 px-3.5 py-2.5'>
                        <span style={{ font: '500 13px var(--font-sans)' }}>{user.name}</span>
                        <button
                            type='button'
                            onClick={logout}
        ...
        <div className='mx-auto w-full max-w-[440px] px-5 py-[clamp(36px,8vh,80px)]'>
            ...
            >
                ...
                <div>
                    ...
                    >
                        {mode === 'signin' ? 'Sign in to Porchlight' : 'Create your account'}
                    </h1>
                    <p className='m-0 text-ink2' style={{ font: '400 14px/1.5 var(--font-sans)' }}>
                ...
                {error && (
                    <div style={{ font: '500 13px var(--font-sans)', color: 'var(--bad)' }}>
                        {error}
                    </div>
                )}
                ...
                <div className='flex items-center gap-3'>
                    ...
                    <span className='mono text-[10px] tracking-[0.14em] text-ink3'>OR</span>
                    <span className='h-px flex-1 bg-line' />
                </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
    return (
        <div>
            <div className='mono mb-1.5 text-[9.5px] tracking-[0.14em] text-ink3'>{label}</div>
            {children}
        </div>
    );
}
